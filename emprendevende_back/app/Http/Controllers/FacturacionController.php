...
class FacturacionController extends Controller
{
    ...
    public function __construct()
    {
     ...
     public function index(){
        ...
        ->join('users as u','u.id','=','f.idUsuario')
        ->join('metodo_pago as m','m.idMETODO_PAGO','=','f.idMETODO_PAGO')
        ->select('f.*','m.codigo','m.tipo','p.nombrePlan','p.detalle','p.costoMensual','p.costoAnual')
        ->get();
            ...
            ->join('users as u','u.id','=','f.idUsuario')
            ->join('metodo_pago as m','m.idMETODO_PAGO','=','f.idMETODO_PAGO')
            ->where('f.idUsuario','=',Auth::user()->id)
            ->select('f.*','m.codigo','m.tipo','p.nombrePlan','p.detalle','p.costoMensual','p.costoAnual','u.name')
            ->get();
            return view('vistasadmin.facturacionplanes.findex',compact('facturacion'));
        }
    }
